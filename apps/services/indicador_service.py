...
def _get_mediciones(sensor: str, request_indicador: IndicadorRequest)->List[Medicion]:
    return mr.get_mediciones(sensor, count=request_indicador.muestras,sort={"fecha":"desc"})
...
def procesar_indicador(request_indicador: IndicadorRequest) -> List[IndicadorResult]:
    ...
    for id_sensor, mediciones in map(lambda s: (s, _get_mediciones(s, request_indicador)), indicador.id_sensores):
