...
class MongoQueryBuilder:
    ...
    def add_slice_field(self,slice_field,page_number,page_size):
        ...
        self.mongo_query.slice_fields.update({slice_field:[page_number*page_size,page_size]})
        return self
    def build(self):
...
def get_by_filter(query:MongoQuery) -> List[Dict]:
    ...
    offset = (query.page_number - 1) * query.items_per_page if query.page_number and query.items_per_page else 0
...
def get_by_id(document_class:Document,id:str)->Dict:
    ...
    builder = MongoQueryBuilder(document_class).add_filter({"_id":ObjectId(id)}).page(0,1).add_exclude_field("_id")
...
def get_by_ids(document_class:Document,ids_list:List[str])->List[Dict]:
    ...
    builder = MongoQueryBuilder(document_class).add_filter({"id":{"$in" :ids_list}})
